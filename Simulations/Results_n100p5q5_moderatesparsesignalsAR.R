...
#### moderate sparse B, AR, empirical risk, p = 5, q = 5, n = 100 ####
...
mats <- matgen(q, rho, type = "AR")

mh <- readRDS("gmcb_mh_n100p5q5_moderatesparseAR.rds")
smn <- readRDS("gmcb_smn_n100p5q5_moderatesparseAR.rds")
mSSL <- readRDS("mSSL_n100p5q5_moderatesparsesignalsAR.rds")
...
#### Frobenius loss ####
hsghs <- read.csv("hsghs_n100p5q5_moderatesparsesignalsAR.csv")
...
loss.omegaols <- sapply(1:iter, 
                        function(x) sum((omega.ols[[x]] - mats$prec.mat)^2))
