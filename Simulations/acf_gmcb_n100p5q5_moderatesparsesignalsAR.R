...
#### ACF comparison, moderate sparse signals, AR ####
...
rho <- 0.7
y.mats <- matgen(q, rho, sigma = 1, type = "AR")
...
write.csv(y, file = "acf_data_n100p5q5_moderatesparsesignalsAR_y.csv",
          row.names = FALSE)

outMH <- gmcb(y = y, x = x, priors = priors, rw.scale = rw.scale, iter = iter,
              algorithm = "MH")
saveRDS(outMH, file = "acf_gmcb_mh_n100p5q5_moderatesparsesignalsAR.rds")
...
outsmn <- gmcb(y = y, x = x, priors = priors, rw.scale = rw.scale, iter = iter,
               algorithm = "SMN")
saveRDS(outsmn, file = "acf_gmcb_smn_n100p5q5_moderatesparsesignalsAR.rds")
