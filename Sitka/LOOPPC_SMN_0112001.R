...
# Note that this code assumes a folder named LOO_GMCBSMN_Sitka1988_0112001 exists in the 
...
forscaling <- readRDS("GMCBSMN_Sitka1988_start_0112001.rds")
iter <- 1e5
...
set.seed(523)
logpostpred_smn_0112001 <- foreach(i = 1:n, .combine = c, .packages = c("GMCB")) %dorng% {
  # fit the model
  outn <- gmcb(y.stand[-i,], x[-i,], initial.values = forscaling$final.values,
              ...
              algorithm = "SMN")
  saveRDS(outn, file = paste("LOO_GMCBSMN_Sitka1988_0112001/LOO_GMCBSMN_Sitka1988_0112001_", i, ".rds", sep = ""))
...
saveRDS(logpostpred_smn_0112001, file = "LOOPPC_SMN_0112001.rds")
