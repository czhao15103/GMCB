...
# Note that this code assumes a folder named LOO_GMCBSMN_Sitka1988_114005 exists in the 
...
forscaling <- readRDS("GMCBSMN_Sitka1988_start_114005.rds")
iter <- 1e5
...
set.seed(625)
logpostpred_smn_114005 <- foreach(i = 1:n, .combine = c, .packages = c("GMCB")) %dorng% {
  # fit the model
  outn <- gmcb(y.stand[-i,], x[-i,], initial.values = forscaling$final.values,
              ...
              algorithm = "SMN")
  saveRDS(outn, file = paste("LOO_GMCBSMN_Sitka1988_114005/LOO_GMCBSMN_Sitka1988_114005_", i, ".rds", sep = ""))
...
saveRDS(logpostpred_smn_114005, file = "LOOPPC_SMN_114005.rds")
